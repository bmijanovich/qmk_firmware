...
//Supports single tap, double tap, and single hold. Favors instant hold when interrupted.
td_state_t single_hold_double_tap_mouse_dance(qk_tap_dance_state_t *state) {
    if (state->count == 1) {
...
//Supports single tap and single hold. Favors instant hold when interrupted.
td_state_t single_hold_single_tap_mouse_dance(qk_tap_dance_state_t *state) {
    if (state->count == 1) {
        if (!state->pressed) return TD_SINGLE_TAP;
        else return TD_SINGLE_HOLD;
    }
...
static td_tap_t lt_btn2_ent_tap_state = {
...
void lt_btn2_ent_finished(qk_tap_dance_state_t *state, void *user_data) {
    lt_btn2_ent_tap_state.state = single_hold_double_tap_mouse_dance(state);
    switch (lt_btn2_ent_tap_state.state) {
        case TD_SINGLE_TAP:
            tap_code16(KC_BTN2);
            break;
        case TD_SINGLE_HOLD:
...
void lt_btn2_ent_reset(qk_tap_dance_state_t *state, void *user_data) {
    ...
    if (lt_btn2_ent_tap_state.state == TD_SINGLE_HOLD) {
        layer_off(1);
    }
    lt_btn2_ent_tap_state.state = TD_NONE;
}




static td_tap_t dragscroll_back_tap_state = {
    .is_press_action = true,
    .state = TD_NONE
};
...
void dragscroll_back_finished(qk_tap_dance_state_t *state, void *user_data) {
    dragscroll_back_tap_state.state = single_hold_single_tap_mouse_dance(state);
    switch (dragscroll_back_tap_state.state) {
        case TD_SINGLE_TAP:
            tap_code16(KC_BTN4);
            break;
        case TD_SINGLE_HOLD:
...
void dragscroll_back_reset(qk_tap_dance_state_t *state, void *user_data) {
    // If the key was held down and now is released then switch off the layer
    if (dragscroll_back_tap_state.state == TD_SINGLE_HOLD) {
        unregister_code16(DRAG_SCROLL);
    }
    dragscroll_back_tap_state.state = TD_NONE;
}




static td_tap_t lt_fwd_tap_state = {
    .is_press_action = true,
    .state = TD_NONE
};
...
void lt_fwd_finished(qk_tap_dance_state_t *state, void *user_data) {
    lt_fwd_tap_state.state = single_hold_single_tap_mouse_dance(state);
    switch (lt_fwd_tap_state.state) {
        case TD_SINGLE_TAP:
            tap_code16(KC_BTN5);
            break;
        case TD_SINGLE_HOLD:
...
void lt_fwd_reset(qk_tap_dance_state_t *state, void *user_data) {
    // If the key was held down and now is released then switch off the layer
    if (lt_fwd_tap_state.state == TD_SINGLE_HOLD) {
        layer_off(2);
    }
    lt_fwd_tap_state.state = TD_NONE;
}
